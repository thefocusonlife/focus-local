...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (!empty($_FILES)) {
        ...
        $errors['image_file'] =
            $_FILES['image']['error'] === 1 ? 'File too big-Resize using Paint ' : '';
    } // If file bigger than limit in php.ini or .htaccess store error message

    // If image was uploaded, get image data and validate
    //if ($temp and $_FILES['image']['error'] == 0) {      // Check file
    if ($temp) {
        // Check file if errors not being generated -- need to determine why TOO BIG images not erroring out
        ...
        $errors['image_file'] .=
            $_FILES['image']['size'] <= MAX_SIZE
                ? ''
                : 'File too big. Resize with Paint or other image utility. Max Size = ' .
                    MAX_SIZE / 1000000 .
                    'mb.'; // Validate file size -doesn't appear to be working
    ...
    */
    ...
    $errors['menu'] = Validate::isMenuId($story['menu_id'], $menus) ? '' : 'Not a valid menu'; // Validate menu
