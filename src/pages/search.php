...
if ($data['term1']) {
    if (!isset($data['term2'])) {                                                // If no search term 
    ...
    if ($data['count'] > 0) { 
        if (!isset($data['term2'])) {                                       // If there are matches
        $data['stories'] = $cms->getStory()->search($data['term1'], $data['show'], $data['from']); // Get matches
        
        } else {
            $data['stories'] = $cms->getStory()->search1($data['term1'], $data['term2'], $data['show'], $data['from']);    
        }
    }
}
