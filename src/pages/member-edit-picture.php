...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (is_uploaded_file($temp) and $_FILES['image']['error'] == 0) {
        // If file OK
        $errors = in_array(mime_content_type($temp), MEDIA_TYPES) ? '' : 'Wrong file type. '; // Validate type
        $extension = strtolower(pathinfo($_FILES['image']['name'], PATHINFO_EXTENSION)); // File extension in lowercase
        $errors .= in_array($extension, FILE_EXTENSIONS) ? '' : 'Wrong file extension. '; // File extension
        $errors .=
