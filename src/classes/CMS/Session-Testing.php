...
class Session // Define Session class
{
    ...
    public function __construct()
    {
        ...
        $this->id = $_SESSION['id'] ?? 3; // Set id property of this object
        ...
        $this->pagelimit = $_SESSION['pagelimit'] ?? 300;
        $this->sorttype = $_SESSION['sorttype'] ?? 1;
        $this->sorttype = $_SESSION['menu_id'] ?? 1;
    ...
    public function create($member, $website)
    {
        ...
        if ($member) {
            ...
            $_SESSION['sorttype'] = (int) ($member['sorttype'] ?? 0); // Add session sort type
            $_SESSION['menu_id'] = $member['menu_id']; // Add member's menu is to session
            ...
            $_SESSION['pagelimit'] = 300; // Add pagelimit to session
            $_SESSION['sorttype'] = 1; // Add sottype to session
            $_SESSION['menu_id'] = $website['menu_id'] ?? 1; // Add default menu is to session
